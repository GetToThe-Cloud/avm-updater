# AVM Updater — GitHub Copilot prompt pack

A ready-to-use set of **GitHub Copilot** custom instructions + prompt files that build
**AvmUpdater**: a PowerShell tool that checks for newer **Azure Verified Module (AVM)**
versions used in your Bicep and Terraform, reports them, requires **approval**, and then
**safely** updates them without breaking your code.

## What's in here

```
.github/
  copilot-instructions.md       # Repo-wide context Copilot loads automatically
  prompts/
    01-scaffold.prompt.md        # Module skeleton + config + stubs
    02-scan.prompt.md            # Find AVM refs + current versions
    03-version-lookup.prompt.md  # Query MCR + Terraform Registry; classify bumps
    04-risk-analysis.prompt.md   # Breaking-change risk (interface diff + changelog)
    05-report.prompt.md          # Markdown overview + JSON manifest
    06-approval.prompt.md        # Approval gate: local / GitHub PR / Azure DevOps PR
    07-updater.prompt.md         # Apply bumps + validate (build/plan) + rollback
    08-orchestrator.prompt.md    # Invoke-AvmUpdate end-to-end
    09-ci.prompt.md              # GitHub Actions + Azure DevOps pipelines
    10-tests.prompt.md           # Pester tests + docs
README.md                        # This file
```

## How to use with GitHub Copilot

1. Copy the **`.github/`** folder into the root of the repo where you want to build the tool.
   (Build it in its own repo, then run the finished tool against your IaC repos.)
2. In **VS Code**, enable prompt/instruction files:
   - `"chat.promptFiles": true` — enables `.github/prompts/*.prompt.md`.
   - `"github.copilot.chat.codeGeneration.useInstructionFiles": true` — auto-loads
     `.github/copilot-instructions.md` into every Copilot Chat request in the repo.
3. Open **Copilot Chat → Agent mode** and run the prompts **in order** by typing
   `/01-scaffold`, then `/02-scan`, … through `/10-tests`. Review and commit after each step.
   - Alternatively, assign each prompt to the **Copilot coding agent** on a GitHub issue, or
     paste the prompt body straight into Copilot Chat.
4. Every prompt inherits the architecture from `copilot-instructions.md`, so you never have to
   re-explain the design.

> Tip: add a `tools:` line to any prompt's frontmatter (e.g.
> `tools: ['codebase','editFiles','runCommands','search']`) and a `model:` line to steer the
> agent further.

---

## AvmUpdater — Usage Guide
